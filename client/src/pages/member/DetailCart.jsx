...
const DetailCart = () => {
    ...
    return (
        <div className="w-full flex flex-col justify-center lg:mb-3 lg:p-10">
            <div className="bg-gray-100">
                <div className="w-full h-[81px] flex items-center mx-auto p-5 justify-between">
                    <div className="flex flex-col gap-2 ">
                        <h3 className="hidden lg:block font-semibold uppercase">My cart</h3>
                        <div
            ...
            {current?.cart?.length > 0 && (
                <>
                    <div className="text-center py-4 lg:hidden">
                        <p className="text-lg font-semibold">Products in your cart</p>
                    </div>
                    <div className="hidden lg:grid w-full grid-cols-10 font-bold border my-5 py-3 mx-auto">
                    ...
                    <div className="px-3 lg:px-0">
                        {current?.cart?.map(el => (
                            <div
                                key={el._id}
                                className="w-full relative border lg:grid lg:grid-cols-10 font-bold my-3 py-3 mx-auto"
                            >
                                <div
                                    className="absolute top-4 right-4"
                                    onClick={async () => {
                                        await apiRemoveCart(el?.product?._id);
                                        dispatch(getCurrent());
                                    }}
                                >
                                    <FaTrashAlt size={28} />
                                </div>
                                <div className="lg:col-span-6 lg:w-full flex justify-between items-center pr-5">
                                    <Link
                                    ...
                                    <div className="lg:hidden">X {el?.quantity}</div>
                                </div>
                                <div className="hidden lg:col-span-3 lg:w-full lg:flex lg:items-center">
                                    <SelectQuantity
                                        quantity={quantity[el._id]}
                                        handleQuantity={number => handleQuantity(el._id, number)}
                                    />
                                </div>
                                <div className="lg:col-span-1 flex items-center gap-2 lg:justify-start justify-end w-full pr-10 lg:pr-0">
                                    <span className="font-normal lg:hidden">Price:</span>
                        ...
                        <div className="p-4 w-full mx-auto flex flex-col justify-center items-end gap-3 border my-3">
                            <span className="flex items-center gap-8 text-sm">
                                ...
                                <span className="text-main font-bold">
                                    {formatMoney(
                                        current?.cart?.reduce(
                                            (sum, el) =>
                                                sum + el?.product?.price * quantity[el._id],
                                            0
                                        )
                                    )}{' '}
                                    VND
                            ...
                            <div className="flex gap-4">
                                <button
                                    ...
                                    className="bg-main text-white px-4 py-2 rounded-md hover:opacity-80 hidden lg:block"
                                >
                                    Update cart
                                </button>
                                <button
            ...
            {current?.cart?.length === 0 && (
                <div className="text-center py-4 flex flex-col lg:block">
